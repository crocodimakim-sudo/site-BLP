...
## 2026-09-07 — Скорость внутренних страниц, llms-full.txt, ежемесячная проверка

**Скорость.** Предзагрузка главной картинки добавлена на `/architect` и `/dealer` (раньше была только на главной). У карточек блока «Объекты» задан точный `sizes` по реальной сетке (3 / 2 / 1 колонка) — браузер брал полноразмерный файл, потому что `sizes` обещал ширину всего экрана. В `.htaccess` добавлен блок Brotli: на текущем хостинге модуля нет, блок игнорируется, включится сам, если хостер его добавит.
Замер Lighthouse mobile: главная **98** (LCP 2,0 с), каталог **100** (LCP 1,3 с), лендинг **99**.

**llms-full.txt.** Новый файл с полными текстами десяти страниц одним документом для AI-ассистентов: `llms.txt` даёт карту и факты, `llms-full.txt` — сам текст. Собирается скриптом `scripts/generate_llms_full.py` (запускать после правок текста на страницах, затем `deploy.bat`). Ссылка добавлена в `llms.txt` и в его копию в `.well-known`.

**Разметка.** В `LocalBusiness` добавлен `hasMap` — ссылка на карту склада в Одинцово.

**Проверка.** Обход всех внутренних ссылок: 31 ссылка и 120 ресурсов, битых нет. Скриншоты 16 страниц — расхождений с прошлым состоянием нет.

**Инструмент.** `05-SEO-И-ОНЛАЙН/audit/monthly-check.py` — ежемесячная проверка одной командой: коды ответов, заголовки, разметка, доступ краулеров, скорость, сравнение с прошлым снимком. Снимки в `audit/snapshots/`, отчёт `audit/REPORT-<дата>.md`. Быстрый прогон без скорости: `python monthly-check.py --fast`.
